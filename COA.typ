...
- 使用主存储器地址的各个地址域：CPU 使用主存储器地址的其中一个域，对暂存在高速缓存中的域请求数据，直接给出数据在高速缓存中的位置，这种情况称为 高速缓存命中
  #[(*_Cache hit_*)]；而 CPU 对没有暂存在高速缓存中的请求数据，会指示出数据将要存放在高速缓存中的位置，这种情况称为 高速缓存缺失
  #[(*_Cache miss_*)]。
...
- 在此基础上，还需要再高速缓存块中定位找出所要求的数据字的存放位置。这项工作可以通过使用主存储器地址中一个称为 字域 #[(*_word field_*)]
  ...
  1. 直接映射的高速缓存
    ...
    - 为了实现直接映射功能，二进制的主存储器地址被划分为了 标记、块、字 三个域。每个域的大小取决于主存储器和高速缓存存储器的物理特性。字 域，又称为 偏移量
      #[(*_offset_*)] 域，用来唯一识别和确定来自某个指定数据块的一个数据字。因此，字域必须具有合适长度的数据位。同样，块
      域必须选择一个唯一的高速缓存块。剩下的字段为 标记
      域。在复制主存储器中的数据块到高速缓存时，标记会随着数据块一起被存储到高速缓存中，并可以通过标记唯一识别和确定数据块。#[(标记确定数据块位置，偏移域确定数据块中的数据位置)]
  2. 全关联高速缓存
    ...
    - 假设不为主存储器中的数据块唯一指定在高速缓存中的位置，而是允许主存储器中的数据块块域存放到高速缓存的任意位置。这种情况下，要找到存储器映射的数据块的唯一方法时搜索全部高速缓存。这样一来，整个高速缓存需要按照
      关联存储器 #[(*_associative memory_*)] 的模式构建，以便 CPU
...
- - ROM
  ...
  - 有 5 种类型不同的基本 ROM 存储器：ROM、PROM、EPROM、EEPROM 和闪存 #[(*_flash memory_*)]。
    - PROM 称为 可编程只读存储器，它是 ROM 一种改进类型，用户可以使用专门的设备对其进行编程。由于 ROM 采用的是硬连线，PROM
